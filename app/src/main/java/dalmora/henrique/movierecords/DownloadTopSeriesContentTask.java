...
public class DownloadTopSeriesContentTask extends AsyncTask<String, Void, Vector<ContentValues>> {
    ...
    public DownloadTopSeriesContentTask(Context context, OnTaskCompleted completeListener, OnTaskPreExecute preExecuteListener) {
        mContext = context;
    ...
    protected Vector<ContentValues> doInBackground(String... params) {
        ...
        try {
            ...
            Uri builtUri_default = Uri.parse(MOVIE_BASE_URL).buildUpon()
                    .appendQueryParameter(T_QUERY_PARAM, params[0])
    ...
    protected void onPostExecute(Vector<ContentValues> vector) {
        ...
        if (vector != null) {
            completeListener.onTaskCompleted(vector, "1", "seriesTOP100");
        } else {
            Log.d(LOG_TAG, "onPostExecute: VECTOR is Null");
            completeListener.onTaskCompleted(null, "1", "seriesTOP100");
        }
    }
    ...
    private Vector<ContentValues> getMovieDataFromJson(String movieJsonStr)
        ...
        Log.d(LOG_TAG, "SERIE STR: " + movieJsonStr);
        try {
